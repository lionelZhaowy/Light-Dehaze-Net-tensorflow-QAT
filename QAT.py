...
FP32_model_path = './FP32_model/best_model/'
INT8_model_path = './INT8_model/'
log_file_path = './INT8_model/log/'
num_of_epochs = 20
...
quant_aware_model.compile(optimizer=optimizer, loss=loss_fn)
quant_aware_model.build(input_shape=(None, 3, 480, 640))

# 训练模型
min_loss = float('inf')  # 最小损失初始化
best_model_path = os.path.join(INT8_model_path, "best_model")
for epoch in range(num_of_epochs):
    ...
    for (hazefree_image, hazy_image) in tqdm(train_dataset):
        ...
        with tf.GradientTape() as tape:
            # 前向传播
            dehaze_image = quant_aware_model(hazy_image, training=True)
            loss = loss_fn(hazefree_image, dehaze_image)
        # 计算梯度
        gradients = tape.gradient(loss, quant_aware_model.trainable_variables)
        # 使用优化器更新模型参数
        optimizer.apply_gradients(zip(gradients, quant_aware_model.trainable_variables))
        ...
        if ((iteration+1) % 200) == 0:
            ...
            print("Average loss now", epoch, ":", avgg_loss.numpy())
            tf.keras.models.save_model(quant_aware_model, os.path.join(INT8_model_path, "Epoch" + str(epoch)))
    ...
    if val_loss < min_loss:
        min_loss = val_loss
        tf.keras.models.save_model(quant_aware_model, best_model_path)
...
# 保存训练结束后的模型
tf.keras.models.save_model(quant_aware_model, os.path.join(INT8_model_path, "final_model"))